...
for (i in 1:1000){
	...
	GRM<- matrix(scan(cov_matrices, n=2000^2, sep=""), nrow=2000, ncol=2000)
	#geno.file<- read.table(geno_name, header = FALSE)
	model0 <- glmmkin(Disease ~ 1, data = pheno, kins = GRM,  family = binomial(link = "logit"))
	glmm.score(model0, infile = geno_name, outfile = paste("glmm.power2.score",i, ".txt", sep=""), infile.ncol.skip = 3)
