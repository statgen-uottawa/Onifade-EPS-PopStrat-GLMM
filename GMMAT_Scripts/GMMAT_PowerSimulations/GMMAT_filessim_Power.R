...
total_files=seq(from=1, to= 2001, by=20)
...
##simulate phenotypes dependent on population since we are testing for type 1 error. 
  pheno_indep <-c()
  pheno1<- c(pheno_indep, rnorm((ind1-1), mean= (0.07 + geno_cand * 0.08854), sd=1))
  pheno2<- c(pheno_indep, rnorm((ind1:ind2), mean= (-0.07 + geno_cand * 0.15), sd=1))
